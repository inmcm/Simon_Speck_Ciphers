...
int main(void){
    ...
    for(int i=0; i < 4; i++) {
        printf("Ciphertext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon64_32_cipher[i]);
        if (ciphertext_buffer[i] != simon64_32_cipher[i]) printf("  FAIL\n");
        else printf("\n");
    } 
    printf("Decryption Test:\n");
    Simon_Decrypt(my_cipher, &simon64_32_cipher, &ciphertext_buffer);
    for(int i=0; i < 4; i++) {
        printf("Plaintext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon64_32_plain[i]);
        if (ciphertext_buffer[i] != simon64_32_plain[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("\n");
    ...
    for(int i=0; i < 6; i++) {
        printf("Ciphertext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon72_48_cipher[i]);
        if (ciphertext_buffer[i] != simon72_48_cipher[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("Decryption Test:\n");
    Simon_Decrypt(my_cipher, &simon72_48_cipher, &ciphertext_buffer);
    for(int i=0; i < 6; i++) {
        printf("Plaintext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon72_48_plain[i]);
        if (ciphertext_buffer[i] != simon72_48_plain[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("\n");
    ...
    for(int i=0; i < 6; i++) {
        printf("Ciphertext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon96_48_cipher[i]);
        if (ciphertext_buffer[i] != simon96_48_cipher[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("Decryption Test:\n");
    Simon_Decrypt(my_cipher, &simon96_48_cipher, &ciphertext_buffer);
    for(int i=0; i < 6; i++) {
        printf("Plaintext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon96_48_plain[i]);
        if (ciphertext_buffer[i] != simon96_48_plain[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("\n");
    ...
    for(int i=0; i < 8; i++) {
        printf("Ciphertext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon96_64_cipher[i]);
        if (ciphertext_buffer[i] != simon96_64_cipher[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("Decryption Test:\n");
    Simon_Decrypt(my_cipher, &simon96_64_cipher, &ciphertext_buffer);
    for(int i=0; i < 8; i++) {
        printf("Plaintext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon96_64_plain[i]);
        if (ciphertext_buffer[i] != simon96_64_plain[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("\n");
    ...
    for(int i=0; i < 8; i++) {
        printf("Ciphertext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon128_64_cipher[i]);
        if (ciphertext_buffer[i] != simon128_64_cipher[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("Decryption Test:\n");
    Simon_Decrypt(my_cipher, &simon128_64_cipher, &ciphertext_buffer);
    for(int i=0; i < 8; i++) {
        printf("Plaintext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon128_64_plain[i]);
        if (ciphertext_buffer[i] != simon128_64_plain[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("\n");
    ...
    for(int i=0; i < 12; i++) {
        printf("Ciphertext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon96_96_cipher[i]);
        if (ciphertext_buffer[i] != simon96_96_cipher[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("Decryption Test:\n");
    Simon_Decrypt(my_cipher, &simon96_96_cipher, &ciphertext_buffer);
    for(int i=0; i < 12; i++) {
        printf("Plaintext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon96_96_plain[i]);
        if (ciphertext_buffer[i] != simon96_96_plain[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("\n");
    ...
    for(int i=0; i < 12; i++) {
        printf("Ciphertext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon144_96_cipher[i]);
        if (ciphertext_buffer[i] != simon144_96_cipher[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("Decryption Test:\n");
    Simon_Decrypt(my_cipher, &simon144_96_cipher, &ciphertext_buffer);
    for(int i=0; i < 12; i++) {
        printf("Plaintext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon144_96_plain[i]);
        if (ciphertext_buffer[i] != simon144_96_plain[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("\n");
    ...
    for(int i=0; i < 16; i++) {
        printf("Ciphertext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon128_128_cipher[i]);
        if (ciphertext_buffer[i] != simon128_128_cipher[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("Decryption Test:\n");
    Simon_Decrypt(my_cipher, &simon128_128_cipher, &ciphertext_buffer);
    for(int i=0; i < 16; i++) {
        printf("Plaintext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon128_128_plain[i]);
    ...
    result = Simon_Init(&my_cipher, Simon_192_128, ECB, simon192_128_key, my_IV, my_counter);
    printf("Encryption Test:\n");
    Simon_Encrypt(my_cipher, &simon192_128_plain, &ciphertext_buffer);
    for(int i=0; i < 16; i++) {
        printf("Ciphertext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon192_128_cipher[i]);
        if (ciphertext_buffer[i] != simon192_128_cipher[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("Decryption Test:\n");
    Simon_Decrypt(my_cipher, &simon192_128_cipher, &ciphertext_buffer);
    for(int i=0; i < 16; i++) {
        printf("Plaintext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon192_128_plain[i]);
        if (ciphertext_buffer[i] != simon192_128_plain[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("\n");
    ...
    for(int i=0; i < 16; i++) {
        printf("Ciphertext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon256_128_cipher[i]);
        if (ciphertext_buffer[i] != simon256_128_cipher[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("Decryption Test:\n");
    Simon_Decrypt(my_cipher, &simon256_128_cipher, &ciphertext_buffer);
    for(int i=0; i < 16; i++) {
        printf("Plaintext Byte %02d: %02x - %02x",i,ciphertext_buffer[i],simon256_128_plain[i]);
        if (ciphertext_buffer[i] != simon256_128_plain[i]) printf("  FAIL\n");
        else printf("\n");
    }
    printf("\n");
